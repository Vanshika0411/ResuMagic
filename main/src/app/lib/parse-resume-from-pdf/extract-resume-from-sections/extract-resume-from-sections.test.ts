...
describe("extract-profile tests - ", () => {
  ...
  it("Email", () => {
    expect(matchEmail(makeTextItem("  hello@RESUMAGIC.org  "))![0]).toBe(
      "hello@RESUMAGIC.org"
    );
  });
  ...
  it("Url", () => {
    expect(matchUrl(makeTextItem("  linkedin.com/in/RESUMAGIC  "))![0]).toBe(
      "linkedin.com/in/RESUMAGIC"
    );
    expect(matchUrl(makeTextItem("hello@RESUMAGIC.org"))).toBeFalsy();
  });
});
